...
def build_cnn_lstm(input_shape, num_classes, dropout_rate=0.4, recurrent_dropout=0.0, l2_reg=0.001, 
            learning_rate=0.001, use_bidirectional=True, units_multiplier=1.0,
    ...
    """
    Build an optimized CNN-LSTM hybrid model with improved architecture and regularization.
    ...
    Returns:
        Compiled Keras CNN-LSTM hybrid model
    """
    # Input layer
    ...
    units_lstm = int(64 * units_multiplier)
    ...
    # LSTM part: LSTM layer (bidirectional if enabled)
    ...
    if use_wks:
        ...
        # Concatenate LSTM output with WKS features
        x = layers.Concatenate()([lstm_output, wks_flat])
    else:
        x = lstm_output
...
def train_cnn_lstm(X_train, y_train, X_val=None, y_val=None, input_shape=None, num_classes=None, 
                 epochs=50, batch_size=32, early_stopping=True, class_weights=None,
                 dropout_rate=0.4, recurrent_dropout=0.0, l2_reg=0.001, learning_rate=0.001, 
                 use_bidirectional=True, units_multiplier=1.0, filters_multiplier=1.0,
    ...
    """
    Train the CNN-LSTM hybrid model with advanced training techniques from the SIAO model.
    ...
    if checkpoint_path is None:
        ...
        checkpoint_path = f"models/checkpoints/cnn_lstm_{timestamp}/best_model.h5"
    ...
    class CompositeMetricCallback(tf.keras.callbacks.Callback):
