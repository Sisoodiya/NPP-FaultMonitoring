...
def build_cnn(input_shape, num_classes, dropout_rate=0.4, l2_reg=0.001, learning_rate=0.001,
    ...
    # Calculate filter sizes based on multiplier
    filters1 = int(64 * filters_multiplier)
    filters2 = int(128 * filters_multiplier)
    filters3 = int(256 * filters_multiplier)
    ...
    if use_wks:
        ...
        # Concatenate CNN output with WKS features
        x = layers.Concatenate()([x, wks_flat])
    
    # Fully connected layers
    x = layers.Dense(
        units=int(128 * filters_multiplier),
        kernel_initializer='he_uniform',
        kernel_regularizer=regularizers.l2(l2_reg)
    )(x)
    x = layers.BatchNormalization()(x)
...
def train_cnn(X_train, y_train, X_val=None, y_val=None, input_shape=None, num_classes=None, 
            ...
            dropout_rate=0.4, l2_reg=0.001, learning_rate=0.001,
            filters_multiplier=1.0, kernel_size=3, pool_size=2,
            use_focal_loss=False, focal_gamma=2.0, focal_alpha=0.25, use_wks=False,
    ...
    """
    Train the CNN model with advanced training techniques from the SIAO model.
    ...
    if checkpoint_path is None:
        ...
        checkpoint_path = f"models/checkpoints/cnn_{timestamp}/best_model.h5"
