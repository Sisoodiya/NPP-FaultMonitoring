...
def build_rnn(input_shape, num_classes, dropout_rate=0.4, recurrent_dropout=0.2, l2_reg=0.001, 
    ...
    """
    Build an optimized RNN model with GRU/LSTM cells, bidirectional layers, dropout, and regularization.
    ...
    # Calculate units based on multiplier
    units_first = int(64 * units_multiplier)
    units_second = int(32 * units_multiplier)
    
    # First RNN layer (bidirectional if enabled)
    ...
    rnn1 = layers.BatchNormalization()(rnn1)
    
    # Second RNN layer (bidirectional if enabled)
    if use_bidirectional:
    ...
    # Add batch normalization for better training stability
    rnn2 = layers.BatchNormalization()(rnn2)
    ...
    if use_wks:
        ...
        x = layers.Concatenate()([rnn2, wks_flat])
    else:
        x = rnn2
...
def train_rnn(X_train, y_train, X_val=None, y_val=None, input_shape=None, num_classes=None, 
             ...
             use_gru=None, use_bidirectional=True, units_multiplier=1.0,
             use_focal_loss=False, focal_gamma=2.0, focal_alpha=0.25, use_wks=False,
             wks_window_size=20, wks_step_size=10, custom_class_weights=False,
             checkpoint_path=None, use_adaptive_learning=True, patience=10,
             min_delta=0.001, monitor='val_loss'):
    """
    Train the RNN model with advanced training techniques from the SIAO model.
    ...
    if checkpoint_path is None:
        ...
        checkpoint_path = f"models/checkpoints/rnn_{timestamp}/best_model.h5"
